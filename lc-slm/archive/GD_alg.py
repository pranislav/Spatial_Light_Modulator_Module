...
def GD_for_hologram_2D(demanded_output: np.array, learning_rate: float, \
                       tolerance: float, max_loops, plot_error: bool=False):
    ...
    error = tolerance + 1
    i = 0
    while error > tolerance or i > max_loops:
        ...
        error = np.sum((output - demanded_output)**2) / space_norm
        print(error)
        error_evolution.append(error)
        i += 1
        # if i%30 == 0: learning_rate *= 2
    if plot_error:
        error_plot(error_evolution, f"learning_rate: {learning_rate}")
