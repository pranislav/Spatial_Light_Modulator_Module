...
data = "paper_shade_01"
Z = (
    np.array(im.open(f"images/incomming_intensity_images/{data}.png").convert("L"))
    / 256
)
y_len, x_len = Z.shape
...
def gaussian_step(xy, a, b, x0, y0):
    ...
    # y0 = y_len // 2
    x, y = xy
    return b + np.exp(-a * ((x - x0) ** 2 + (y - y0) ** 2)) * heaviside_circle(
        x, y, x0, y0, R_cam
    )
...
gaustep = (
    256 * (b_fit + gaussian_step((x_flat, y_flat), a_fit, b_fit, x0_fit, y0_fit))
).reshape(Z.shape)
# im.fromarray(gaustep).show()


def gaussian_step_coord(x, y, a, x0, y0, R):
    return (np.exp(-a * ((x - x0) ** 2 + (y - y0) ** 2))) * heaviside_circle_coord(
        x, y, x0, y0, R
    )
...
def create_intensity_mask(a):
    ...
    for y in range(intensity_mask_arr.shape[0]):
        ...
        for x in range(intensity_mask_arr.shape[1]):
            x_ = x * convert
            intensity_mask_arr[y, x] = 255 * gaussian_step_coord(
                x_, y_, a, x0 * convert, y0 * convert, R_slm * convert
            )
    im.fromarray(intensity_mask_arr).convert("L").save(
        f"incomming_intensity_images/{data}_b=0_intensity_mask.png"
    )
