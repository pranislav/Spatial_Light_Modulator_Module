    ...
    while True:
        ...
        if command[0:2] == "cm":
            if len(command) == 2:
                ...
                if name is not None:
                    display_with_mask(
                        window, name, directory, mask_arr, args.correspond_to2pi
                    )
                continue
            maybe_mask_arr = set_mask(command[3:])
            if maybe_mask_arr is not None:
                mask_arr = maybe_mask_arr  # TODO: delete this?
            if name is not None:
                display_with_mask(
                    window, name, directory, mask_arr, args.correspond_to2pi
                )
        ...
        if command[:2] == "s ":
            if " " not in command[2:]:
                ...
                wait = float(wait)
            display_holograms_in_sequence(
                window, dir, wait, mask_arr, args.correspond_to2pi
            )
            continue
        name = command
...
def display_holograms_in_sequence(window, dir, wait, mask_arr, ct2pi):
    ...
    display_with_mask_partial = partial(
        display_with_mask, window, directory=dir, mask_arr=mask_arr, ct2pi=ct2pi
    )
    while i < i_max:
        i += 1
        ...
        while True:
            if keyboard.is_pressed("space"):
                time.sleep(
                    key_wait
                )  # to avoid multiple toggles from a single key press due to key repeat
                i, quit_func = stop_mode(
                    display_with_mask_partial, i, i_max, key_wait, micro_wait, dir
                )
                break
...
def display_instant_wavefront_correction_holograms(window):
    ...
    while True:
        get_params(params)
        sample_hologram_2pi = wfc.deflect_2pi(
            params["deflect"], params["correspond_to_2pi"]
        )
        sample_hologram = wfc.convert_2pi_hologram_to_int_hologram(
            sample_hologram_2pi, params["correspond_to_2pi"]
        )
        hologram = im.fromarray(np.zeros((c.slm_height, c.slm_width)))
        reference_position = e.real_subdomain_position(
            params["reference_position"], params["subdomain_size"]
        )
        subdomain_position = e.real_subdomain_position(
            params["subdomain_position"], params["subdomain_size"]
        )
        hologram = wfc.add_subdomain(
            hologram, sample_hologram, reference_position, params["subdomain_size"]
        )
        if params["phase_shift"] != 0:
            sample_hologram = (
                wfc.deflect(params["deflect"], params["correspond_to_2pi"])
                + params["phase_shift"]
            )
        hologram = wfc.add_subdomain(
            hologram, sample_hologram, subdomain_position, params["subdomain_size"]
        )
...
def display_with_mask(window, name, directory, mask_arr, ct2pi):
    ...
    if mask_arr is not None:
        wfc.display_image_on_external_screen(
            window, mask_hologram(path, mask_arr, ct2pi)
        )
        return
    if path[-4:] == ".npy":
...
def print_help():
    ...
    print(
        "- s <directory> <wait_time> - display holograms in sequence from specified directory. wait_time is time between frames in seconds. if not specified, wait_time is 0. press space to pause, left and right to navigate, esc to quit"
    )
...
def set_dir(directory):
    if not (os.path.isdir(directory)):
        print(
            "specified directory does not exist. make sure you are in project root and path is correct."
        )
        return None
...
def mask_hologram(path, mask_arr, ct2pi):
    ...
    if ext == ".npy":
        ...
        corrected_hologram_arr = (
            hologram_arr + (mask_arr / (2 * np.pi) * ct2pi)
        ) % ct2pi
...
if __name__ == "__main__":
    ...
    mask_help = f"""Type name of the wavefront correction mask
        to be added to displayed holograms or leave blank for no mask.
        Mask has to be in .npy format in {mask_dir}.
        Mask values has to be in range [0, 2*pi).
        """
    directory_help = """path (from project root) to directory
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help=f"{help_messages_wfc.ct2pi}. you can change this parameter later by typing 'ct2pi <value>' in the console.",
    )
    parser.add_argument(
        "mask_name",
        nargs="?",
        ...
        type=str,
        help=mask_help
        + " you can change mask later by typing 'cm <mask_name>' in the console.",
    )
    parser.add_argument(
        "-d",
        "--directory",
        default=default_hologram_dir,
        type=str,
        help=directory_help
        + " you can change directory later by typing 'cd <directory>' in the console.",
    )
