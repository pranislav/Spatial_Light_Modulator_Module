...
def main(args):
    ...
    list_of_position_lists = create_list_of_position_lists(
        args.parametrization, args.number_of_frames, args.rescale_parameter
    )
...
def two_circulating_dots_quarterized(n: int):
    ...
    return [
        (
            ...
            h_ * (1 / 2 + 1 / 3 * np.sin(t))),
        (
            w_ * (1 / 2 + 1 / 3 * np.cos(t + np.pi / 2)),
            h_ * (1 / 2 + 1 / 3 * np.sin(t + np.pi / 2)),
        ),
    ]
...
def two_circulating_dots(n: int):
    ...
    return [
        (
            c.w * (1 / 2 + 1 / 3 * np.cos(t)),
            c.h * (1 / 2 + 1 / 3 * np.sin(t))),
        (
            c.w * (1 / 2 + 1 / 3 * np.cos(t + np.pi / 2)),
            c.h * (1 / 2 + 1 / 3 * np.sin(t + np.pi / 2)),
        ),
    ]


def create_list_of_position_lists(
    parametrization: function, number_of_frames: int, rescale_parameter: int = 1
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        choices=[
            "circulating_dot_quarterized",
            "two_circulating_dots_quarterized",
            "two_circulating_dots",
        ],
        default="circulating_dot_quarterized",
        help="parametrization of the paths of the dots",
    )
    parser.add_argument(
        "-r",
        "--rescale_parameter",
        ...
        type=float,
        default=1,
        help="rescales parameter (faster (> 1) or slower (< 1)) motion",
    )
    parser.add_argument(
        "-n", "--number_of_frames", metavar="INT", type=int, default=360
    )
