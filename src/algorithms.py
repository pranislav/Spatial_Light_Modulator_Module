...
def gerchberg_saxton(demanded_output, args):
    ...
    """
    incomming_intensity = (
        np.ones(demanded_output.shape)
        if args.incomming_intensity == "uniform"
        else np.array(im.open(args.incomming_intensity))
    )
...
def gradient_descent(demanded_output: np.array, args):
    ...
    error_evolution = []
    norm = np.amax(demanded_output)
    input = make_initial_guess(
        args.initial_guess, incomming_amplitude, demanded_output, args.random_seed
    )
    error = args.tolerance + 1
    i = 0
    mask = 1 + args.white_attention * demanded_output / 255
    if args.print_info:
        print("computing hologram")
    while error > args.tolerance and i < args.max_loops:
        med_output = fft2(input / abs(input) * incomming_amplitude)
        output_unnormed = abs(med_output) ** 2
        output = output_unnormed * norm / np.amax(output_unnormed)
        dEdF = (
            ifft2(mask * med_output * (output - demanded_output)) * incomming_amplitude
        )
        ...
        if args.gif and i % args.gif_skip == 0:
            if args.gif_type == "h":
                img = im.fromarray(
                    complex_to_real_phase(input / abs(input), args.correspond_to2pi)
                )
            if args.gif_type == "i":
...
def make_initial_guess(initial_guess_type, incomming_amplitude, demanded_output, seed):
    ...
    if initial_guess_type == "random":
        return np.array(
            [
                [np.exp(1j * 2 * np.pi * random.random()) for _ in range(w)]
                for _ in range(h)
            ]
        )
    if initial_guess_type == "old":
        return np.array(
            [
                [
                    (np.sqrt(random.random()) + 1j * np.sqrt(random.random()))
                    for _ in range(w)
                ]
                for _ in range(h)
            ]
    ...
    if initial_guess_type == "unnormed":
        return (
            np.array(
                [
                    [random.random() + 1j * random.random() for _ in range(w)]
                    for _ in range(h)
                ]
            ) - 0.5
        ) * 2
    if initial_guess_type == "zeros":
        return np.array(
            [
                [np.exp(1j * 2 * np.pi * random.random()) / 100 for _ in range(w)]
                for _ in range(h)
            ]
        )
    if initial_guess_type == "ones":
        ...
        return incomming_amplitude * np.exp(
            1j * np.angle(ifft2(np.sqrt(demanded_output)))
        )
    else:
