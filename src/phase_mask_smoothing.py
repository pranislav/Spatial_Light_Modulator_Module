...
def main(args):
    ...
    upscaled_unwrapped_mask = im.fromarray(unwrapped_mask).resize(
        (c.slm_width, c.slm_height), resample=resample
    )
    upscaled_unwrapped_mask = np.array(upscaled_unwrapped_mask)
    time_name = time.strftime("%Y-%m-%d_%H-%M-%S")
    save_smoothed_mask_unwrapped(
        copy.deepcopy(upscaled_unwrapped_mask),
        args.phase_mask_name[:-4],
        time_name,
        args.source_dir,
    )
    save_smoothed_mask(
        upscaled_unwrapped_mask,
        args.phase_mask_name[:-4],
        time_name,
        args.correspond_to_2pi,
        args.source_dir,
    )
...
def main_blur(args):
    ...
    small_phase_mask = shrink_phase_mask(phase_mask, args.subdomain_size)
    unwrapped_mask = unwrap_phase_picture(small_phase_mask, args.correspond_to_2pi)
    original_size_unwrapped_mask = wfc.expand_phase_mask(
        unwrapped_mask, args.subdomain_size
    )
    blurred_unwrapped_mask = circular_box_blur(
        original_size_unwrapped_mask, args.subdomain_size // 2
    )
    time_name = time.strftime("%Y-%m-%d_%H-%M-%S")
    save_smoothed_mask_unwrapped(
        copy.deepcopy(blurred_unwrapped_mask), args.phase_mask_name, time_name
    )
    save_smoothed_mask(
        blurred_unwrapped_mask, args.phase_mask_name, time_name, args.correspond_to_2pi
    )
...
def save_smoothed_mask_unwrapped(smoothed_mask, name, time_name, source_dir):
    ...
    smoothed_mask_unwrapped.convert("L").save(
        f"{source_dir}/{name}_smoothed_unwrapped_{time_name}.png"
    )
...
def unwrap_phase_picture(phase_mask, correspond_to_2pi):
    ...
    unwrapped_phase_mask = transform_to_color_values(
        unwrapped_phase_mask, correspond_to_2pi
    )
...
def circular_hole_inclusive(shape):
    ...
    mask = np.array(
        [
            [0 if (i - i0) ** 2 + (j - j0) ** 2 < R**2 else 1 for j in range(w)]
            for i in range(h)
        ]
    )
    return mask
...
def show_negative(arr):
    binary = np.array(
        [[0 if a > 0 else 255 for a in arr[i]] for i in range(arr.shape[0])]
    )
    im.fromarray(binary).convert("L").show()


if __name__ == "__main__":
    description = """smooth a phase mask by unwrapping it and resampling it
    to the SLM size using linear or bicubic interpolation
    """
    ...
    parser.add_argument(
        "phase_mask_name",
        type=str,
        help=f"name of a phase mask in directory {source_dir}",
    )
    parser.add_argument(
        "-ct2pi",
        "--correspond_to_2pi",
    ...
    parser.add_argument(
        ...
        default="bilinear",
        help="smoothing method used to upscale the unwrapped phase mask",
    )
